...
export default function FavoritesRewriteScreen() {
  ...
  const loadFavorites = useCallback(() => {
    getFavorites("rewrite").then(setFavorites);
  }, []);

  useFocusEffect(loadFavorites);

  async function handleRemove(id: string) {
    await removeFavorite(id, "rewrite");
    setFavorites((current) => current.filter((item) => item.id !== id));
    Alert.alert("Removed", "Saved rewrite removed.");
  }
    ...
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.headerRow}>
          <View>
            <Text style={styles.eyebrow}>Transformation</Text>
            <Text style={styles.title}>Saved Rewrites</Text>
          </View>
        ...
        {favorites.length === 0 ? (
          <EmptyState
            title="No saved rewrites yet"
            message="Tap Save on any rewrite to keep it here."
            icon="star-outline"
          />
        ) : (
